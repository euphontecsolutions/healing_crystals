...
if(isset($HTTP_GET_VARS['set_flag'])){
    $date = date("Y-m-d H:i:s");
    tep_db_query("INSERT INTO `customer_redirect_payment` (`id`, `customer_id`, `flag`, `created_date`) VALUES (NULL, '" . (int)$customer_id . "', 1,'" .$date . "')");
    
}else{
    tep_db_query("DELETE FROM `customer_redirect_payment` WHERE `customer_redirect_payment`.`customer_id` = '" . (int)$customer_id . "'");
...
/*==================================================================================*/
  require 'push.php';
      ...
      if(isset($orders_array['orders_status'])) {
...
$authorize_status_query = tep_db_query("select o.orders_status, o.cc_number, o.payment_method, o.cc_cvn, o.cc_expires, o.authorize_trx_id, o.inventory_update from orders o where o.orders_id = '".(int)$HTTP_GET_VARS['order_id']."'");
    ...
    $_SESSION["order_id"] = $HTTP_GET_VARS['order_id'];
...
if($products['products_model'] == 'VGIFT'){
  ...
  $content = 'checkout_success_mobile';
