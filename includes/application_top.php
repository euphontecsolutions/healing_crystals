...
//* 2-MAY-2012, (MA) add ability for customers to easily reorder , #557 , Bof
    if (DISPLAY_CART == 'true' && basename($PHP_SELF)!='account_history_info.php') {
      $goto =  FILENAME_SHOPPING_CART;
	  if($HTTP_GET_VARS['order_id']!=''){
		$goto='account_history_info.php';
                                    ...
                                    if (tep_check_in_stock(tep_get_uprid($HTTP_POST_VARS['products_id'], $HTTP_POST_VARS['id']), $HTTP_POST_VARS['qty'])){
                                        ...
                                        if($check_unit_per_sale != 0){
                                            if($check_unit_per_sale >= $HTTP_POST_VARS['qty'] ){
                                                $cart->add_cart($HTTP_POST_VARS['products_id'], $HTTP_POST_VARS['qty'], $HTTP_POST_VARS['id']);
                                            }else{
                                                $_SESSION['unit_per_sale_error'][$HTTP_POST_VARS['products_id']] = $check_unit_per_sale;
                                                $cart->add_cart($HTTP_POST_VARS['products_id'], $check_unit_per_sale, $HTTP_POST_VARS['id']);
                                            }
                                        }else{
                                            $cart->add_cart($HTTP_POST_VARS['products_id'], $cart->get_quantity(tep_get_uprid($HTTP_POST_VARS['products_id'], $HTTP_POST_VARS['id'])) + $HTTP_POST_VARS['qty'], $HTTP_POST_VARS['id']);
                                        }                                    
                                    } else{
