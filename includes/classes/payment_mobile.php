...
// class constructor
    ...
    function payment_mobile($module = '') {
...
// Show either normal payment modules or free payment module when Free Shipping Module is On
          ...
          //print_r($cart->weight);
          if (tep_get_configuration_key_value('MODULE_PAYMENT_FREECHARGER_STATUS') and ($cart->show_total()==0 and $cart->weight==0)) {
    ...
    function javascript_validation() {
      ...
      if (is_array($this->modules)) {
        ...
        $js = '<script language="javascript" type="text/javascript"><!-- ' . "\n" .
              ...
              '  var coupon_value = document.checkout_payment.gv_redeem_code.value;' . "\n" .
        ...
        $js .= "\n" . '  if (payment_value == null && submitter != 1 && coupon_value == "") {' . "\n" . // ICW CREDIT CLASS Gift Voucher System
...
//  ICW CREDIT CLASS Gift Voucher System Line below amended
               ...
               '  if (error == 1 && submitter != 1 && coupon_value == "") {' . "\n" .
      ...
      if (is_array($this->modules)) {
        ...
        while (list(, $value) = each($this->modules)) {
          ...
          if ($GLOBALS[$class]->enabled) {
            ...
            if ($class == 'paypal_wpp') {
              $selection = $GLOBALS[$class]->express_selection();
...
//var_dump($GLOBALS[$this->selected_module]);
//die();
