<?php
/*
  $Id: order_total.php,v 1.3.2.4 2003/05/05 12:46:53 wilt Exp $
      ...
      if (is_array($this->modules)) {
        ...
        while (list(, $value) = each($this->modules)) {
          ...
          if ($GLOBALS[$class]->enabled) {
            ...
            for ($i=0; $i<$size; $i++) {
              $output_string .= '              <tr>' . "\n" .
                                '                <td align="right" class="main  headingpay">' . $GLOBALS[$class]->output[$i]['title'] . '</td>' . "\n" .
                                '                <td align="right" class="main  paymentpay">' . $GLOBALS[$class]->output[$i]['text'] . '</td>' . "\n" .
                                '              </tr>';
            }
          }
        }
...
//
    function credit_selection() {
      ...
      if (MODULE_ORDER_TOTAL_INSTALLED) {
        ...
        if(basename($_SERVER['PHP_SELF']) == 'checkout_payment.php'){
              //$header_string = '<tr>' . "\n";
       // $header_string .= '   <td><table border="0" width="100%" cellspacing="0" cellpadding="2">' . "\n";
           // $header_string .= '      <tr>' . "\n";
          //$header_string .= '        <td class="mainCP">' . TABLE_HEADING_CREDIT . '</td>' . "\n";
          //$header_string .= '      </tr>' . "\n";
           //$header_string .= '    </table></td>' . "\n";
        //$header_string .= '  </tr>' . "\n";
         $header_string .= '<tr>' . "\n";
        $header_string .= '   <td><table border="0" width="100%" cellspacing="0" cellpadding="0" class="infoBox">' . "\n";
    ...
    function get_order_total_main($class, $order_total) {
     ...
     function redeem_coupon() {
      if (MODULE_ORDER_TOTAL_INSTALLED) {
        ...
        while (list(, $value) = each($this->modules)) {
          $class = substr($value, 0, strrpos($value, '.'));
          if ( ($GLOBALS[$class]->enabled && $GLOBALS[$class]->code == 'ot_coupon') ) {
            $output_string = $GLOBALS[$class]->redeem_coupon();
          }
        }
      }
      return $output_string;
