...
/*    print_r($_GET);
    die();*/
    $flag=$_GET['error_message'];
    $flag2 = $_GET['payment_error'];
    if($flag || $flag2){
        tep_redirect(tep_href_link('checkout_payment_mobile.php'.'?error_message='.$flag.'&payment_error='.$_GET['payment_error'].'&error='.$GET['error'].'&authorizenet_cc_owner='.$GET['authorizenet_cc_owner'].'&authorizenet_cc_expires_month='.$GET['authorizenet_cc_expires_month'].'&authorizenet_cc_expires_year='.$GET['authorizenet_cc_expires_year']));
    }else{
      tep_redirect(tep_href_link('checkout_payment_mobile.php'));
    }
  }
  ...
  if ( (STOCK_CHECK == 'true') && (STOCK_ALLOW_CHECKOUT != 'true') ) {
    ...
    for ($i=0, $n=sizeof($products); $i<$n; $i++) {
      if (tep_check_stock($products[$i]['id'], $products[$i]['quantity'])) {
        tep_redirect(tep_href_link(FILENAME_SHOPPING_CART,'action=adjust_inventory'));
        break;
      }
    }
  }
...
// load all enabled payment modules
  ...
  require(DIR_WS_TEMPLATES . TEMPLATE_NAME . '/main_page.tpl.php');
