...
//          $products[$i][$option]['products_options_values_name'] = $attributes_values['products_options_values_name'];
    ...
    for ($i=0, $n=sizeof($products); $i<$n; $i++) {$products_name='';
      if (($i/2) == floor($i/2)) {
...
?>
	<table id="cart" class="table table-condensed" >
	  ...
	  ?>
							<td data-th="Price" style="background: white;"><?php echo $unit_special_price . '<b>' . $currencies->display_price($products[$i]['final_price'], tep_get_tax_rate($products[$i]['tax_class_id'])) . '</b>';?></td>
							<td data-th="Quantity" style="background: white;">
								<?php echo tep_draw_input_field('cart_quantity[]', $products[$i]['quantity'], 'size="1" maxlength="4" style="width: 8%;"'). tep_draw_hidden_field('products_id[]', $products[$i]['id']);?>
							
								<button class="btn btn-info btn-sm" onclick="document.forms['cart_quantity'].submit();" ><i class="fa fa-refresh"></i></button>
								
								<a href="<?php echo $_SERVER['PHP_SELF'] . '?action=add_to_wishlist&products_id='.$products[$i]['id'].'&qty='.$products[$i]['quantity'];?>" >
								    <input type="button" class="btn btn-info btn-sm" border="0" style="background-color:#777; width: 31%;

font-size: 9px;"  alt="Remove from Shopping Cart and Add to Wish List" title=" Remove from Shopping Cart and Add to Wish List " value="Move to Wishlist"/>
							</a>
							<a href="<?php echo $_SERVER['PHP_SELF'] . '?action=remove_product&products_id='.$products[$i]['id'].'&qty='.$products[$i]['quantity'];?>" >
								 <input type="button"  class="btn btn-danger btn-sm" style="float: left;position: absolute; background-color:#777;" value="X"/><i class="fa fa-trash-o"></i>
								 </a>
							</td>
