<?php
  ...
  $mobile_metaQuery = tep_db_query("select app_id from customer_meta_mobile where app_id = '".$app_id."'");
  $app_meta_dataArray = tep_db_fetch_array($mobile_metaQuery);
  $date = date("Y-m-d H:i:s");
  if($app_meta_dataArray==''){
  tep_db_query("insert into customer_meta_mobile (app_id, created_date) values ('" .$app_id. "', '".$date."')");
  }else{
...
$email_text = str_replace($variableToBeReplaced,$variableToBeAdded,$emailTemplateArray['page_and_email_templates_content']);
     ...
     tep_db_query("update customer_meta_mobile set flag = '1', customer_id = '".(int)$customer_id."' where app_id = '" .$app_id . "'");
