...
function get_sub_category(){
    ...
    if ($postdata=='') {
       /* $request = json_decode($postdata);*/
		...
		$category_level ='1'; /*$request->category_level;*/
		$slider = '3'; /*$request->slider;*/
...
function user_login(){
    ...
    function user_register(){
        ...
        if (isset($postdata)) {
           ...
           /* $data = array();
            $data['firstname'] = 'damu@euphontec.com';
            $data['lastname']='2';
            $data['password']='3';
            $data['confirmation']='4';
            $data['email_address']='5';
            $data['telephone']='6';
            $data['company']='7';
            $data['street_address']='8';
            $data['suburb']='9';
            $data['city']='1';
            $data['state']='2';
            $data['postcode']='3';
            $data['country']='4';
            $data['company2']='5';
            $data['street_address2']='6';
            $data['suburb2']='7';
            $data['city2']='8';
            $data['state2']='9';
            $data['postcode2']='1';
            $data['country2']='2';*/
             header('Location: https://www.copy.healingcrystals.com/create_account_mobile.php?firstname='.$firstname.'&lastname='.$lastname.'&password='.$password.'&confirmation='.$confirmation.'&email_address='.$email_address.'&telephone='.$telephone.'&company='.$company.'&street_address='.$street_address.'&suburb='.$suburb.'&city='.$city.'&state='.$state.'&postcode='.$postcode.'&country='.$country.'&company2='.$company2.'&street_address2='.$street_address2.'&suburb2='.$suburb2.'&city2='.$city2.'&state2='.$state2.'&postcode2='.$postcode2.'&country2='.$country2);
                exit();
     ...
     function user_register3(){
      /*   require('../includes/configure.php');
        ...
        if (isset($postdata)) {
            $request                = json_decode($postdata);
            $firstname                  = 'arun@euphontec.com';
            $lastname='2';
            $password='3';
            $confirmation='4';
            $email_address=trim('arun@euphontec.com');
            $telephone='6';
            $company='7';
            $street_address='8';
            $suburb='9';
        	...
        	$sql_check                    = "SELECT * FROM `customers` WHERE `customers_email_address` = '".$email_address."'";
    		$results                 = $conn->query($sql_check);
    	/*	echo $sql_check;
             print_r($results);
    		die();*/
    		$module_shipping_usps_userid = '193WWWHE6727';
    /*		 if((int)$country == '223'){
...
<Address2>'.$street_address.'</Address2><City>'.$city.'</City><State>'.$state.'</State><Zip5>'.$postcode.'</Zip5><Zip4></Zip4></Address></AddressValidateRequest>';
        ...
        $http = new httpClient();               
                 ...
                 if (preg_match('/<Error>/', $respBody)) {
                     $categoryArr['status']	= "error";
    		      $categoryArr['error']	= "Your address failed USPS Adrress Verification, Please enter correct Address";
                }
    }*/
    		if ($results->num_rows < 0) {
    		      $categoryArr['status']	= "error";
                ...
                if($customer_id){
                    ...
                    $store_owner       = "HealingCrystals.com ";
                    $store_owner_email_address = "contact@healingcrystals.com";
                    $email_template_new_customer = "EMAIL_TEMPLATE_NEW_CUSTOMER";
                    $templatesq = "select page_and_email_templates_content from page_and_email_templates where page_and_email_templates_key = 'EMAIL_TEMPLATE_NEW_CUSTOMER'";
                    $emailTemplateQuery =  $conn->query($templatesq);
                    $emailTemplateArray = $emailTemplateQuery->fetch_assoc();
                    ...
                    mail($name, $email_address, $email_subject, $email_text,true);
                }
                $conn->close(); 
    		}
