<?php
define("base_url", "test.healingcrystals.com/"); 
							       ...
							       $linked_product_query = $conn->query($linked_product_query);
							        if ($linked_product_query->num_rows > 0) {
							          ...
							          if($products_options_values['products_attributes_units']>0){
							            ...
							            'price'=>number_format($products_options_values['products_attributes_special_price_1'], 2, '.', ''),'quantity'=>$products_options_values['products_attributes_units'],'option_value'=>$products_options_values['products_options_values_id']);
				...
				/*if($category_id=='NQ'){
					...
					$productsIdsString='';
					    ...
					    $productsIdsString = $prolist99;
    					  ...
    					  $sql="select psp.property_value, p.products_id, p.products_image, pd.products_name from products p, products_description pd, products_specific_property psp, products_to_stones p2st where psp.products_id = p.products_id and pd.products_id = p.products_id and pd.language_id = '1' and lower(psp.property_name) = 'shape' and p.products_status = 1 and (p.products_model like 'N%' or p.products_model like 'DN%') and p2st.products_id = p.products_id and p2st.stone_name_id = '203'and p.products_id in (".$productsIdsString.") group by psp.property_value order by psp.property_value ASC";
    					    $result = $conn->query($sql);
    					    if ($result->num_rows > 0) {
    						    while($rows = $result->fetch_assoc()) {
        ...
        //--------------------
         ...
         $result_rows = $conn->query($listing_sqlcount);
           ...
           $startindex  = ($current_page * $perpage) - $perpage;
			      ...
			      $listing_sql = "SELECT a2t.*,ad.articles_url, a.articles_id, a.authors_id, a.articles_date_added, ad.articles_name, ad.articles_head_desc_tag, ad.articles_description, au.authors_name, sn.detailed_mpd,sn.stone_name_id, a2t.topics_id, SUBSTRING( ad.articles_name ,1 ,1) AS start_letter FROM (articles a, articles_description ad, articles_to_topics a2t) LEFT JOIN authors au ON a.authors_id = au.authors_id LEFT JOIN stone_names sn ON a.articles_id = sn.summary_mpd WHERE (a.articles_date_available IS NULL OR to_days(a.articles_date_available) <= to_days(now())) AND a.articles_status = '1' AND a.articles_id = a2t.articles_id AND ad.articles_id = a2t.articles_id AND ad.language_id = '1' AND a2t.topics_id = '3' ORDER BY ad.articles_name limit $startindex,$perpage";
		...
		if ($result->num_rows > 0) {
		    while($row = $result->fetch_assoc()) {
			    ...
			    if($password_check==1){
			        ...
			        $sql = "INSERT INTO customer_mobile_app (cma_customers_id, cma_customers_app_id, cma_customers_token, cma_customers_location) VALUES ('".$row['customers_id']."', '".$user_app_id."', '".$app_key."', '".$location."')";
                    $conn->query($sql);
    ...
    function edit_account(){
		...
		if ($auth_result->num_rows > 0) {
           ...
           /* $firstname                  = 'damu@euphontec.com';
    		...
    		$address_result    = $conn->query($auth);
		if ($address_result->num_rows > 0) {
		    $address1=$address_result[0];
		    $address2=$address_result[1];
		    
        	    $sql = "update customer set customers_firstname = '" .$firstname . "', customers_lastname = '" .$lastname . "', customers_email_address = '" .$email_address . "', customers_telephone = '" .$telephone . "'  where address_book_id = '" . $address1 . "'";
                $conn->query($sql);
		    
		    if($address1){
                $sql1 = "update set address_book (entry_firstname = '" .$firstname . "', entry_lastname = '" .$lastname . "', entry_street_address = '" .$street_address . "', entry_postcode = '" .$postcode . "', entry_city = '" .$city . "', entry_country_id = '" .$country . "',entry_company = '" .$company . "',entry_suburb = '" .$suburb . "' ,entry_state = '" .$state . "' where address_book_id = '" . $address1 . "'";
                $conn->query($sql1);
		    }else{
		    ...
		    if($address2){
                $sql2 = "update set address_book (entry_firstname = '" .$firstname . "', entry_lastname = '" .$lastname . "', entry_street_address = '" .$street_address2 . "', entry_postcode = '" .$postcode2 . "', entry_city = '" .$city2 . "', entry_country_id = '" .$country2 . "',entry_company = '" .$company2 . "',entry_suburb = '" .$suburb2 . "' ,entry_state = '" .$state2 . "' where address_book_id = '" . $address2 . "'";
                $conn->query($sql2);
		    }else{
		   ...
		   $sql1 = "SELECT entry_street_address, entry_postcode, entry_city, entry_country_id,entry_company,entry_suburb,entry_zone_id,entry_state FROM  `address_book` WHERE `customers_id`='".$user_id."'";
		    ...
		    if ($results->num_rows > 0) {
		        while($rows = $results->fetch_array()) {
		            ...
		            $entry_street_address   = $rows[0]['entry_street_address'];
		            $entry_postcode = $rows[0]['entry_postcode'];
		            $entry_city    = $rows[0]['entry_city'];
		            $entry_country_id    = $rows[0]['entry_country_id'];
		            $entry_company   = $rows[0]['entry_company'];
		            $entry_suburb = $rows[0]['entry_suburb'];
		            $entry_zone_id    = $rows[0]['entry_zone_id'];
		            $entry_state    = $rows[0]['entry_state'];
		            
		            $entry_street_address2   = $rows[1]['entry_street_address'];
		            $entry_postcode2 = $rows[1]['entry_postcode'];
		            $entry_city2    = $rows[1]['entry_city'];
		            $entry_country_id2    = $rows[1]['entry_country_id'];
		            $entry_company2   = $rows[1]['entry_company'];
		            $entry_suburb2 = $rows[1]['entry_suburb'];
		            $entry_zone_id2    = $rows[1]['entry_zone_id'];
		            $entry_state2    = $rows[1]['entry_state'];
		        }
		    }
...
function get_token(){
    ...
    if (isset($postdata)) {
		...
		if ($result->num_rows > 0) {
		    ...
		    $app_key    = customer_pass_encryption($app_id);
    		$sql = "INSERT INTO customer_mobile_app (cma_customers_id, cma_customers_app_id, cma_customers_token, cma_customers_location) VALUES ('".$user_id."', '".$app_id."', '".$app_key."', '".$location."')";
                    $conn->query($sql);
...
function mpd_filter(){
    ...
    if (isset($postdata)) {
		...
		$listing_sql = "SELECT a2t.*,ad.articles_url, a.articles_id, a.authors_id, a.articles_date_added, ad.articles_name, ad.articles_head_desc_tag, ad.articles_description, au.authors_name, sn.detailed_mpd,sn.stone_name_id, a2t.topics_id, SUBSTRING( ad.articles_name ,1 ,1) AS start_letter FROM (articles a, articles_description ad, articles_to_topics a2t) LEFT JOIN authors au ON a.authors_id = au.authors_id LEFT JOIN stone_names sn ON a.articles_id = sn.summary_mpd WHERE (a.articles_date_available IS NULL OR to_days(a.articles_date_available) <= to_days(now())) AND a.articles_status = '1' AND a.articles_id = a2t.articles_id AND ad.articles_id = a2t.articles_id AND ad.language_id = '1' AND a2t.topics_id = '3' and ad.articles_name LIKE '".$filter."%' ORDER BY ad.articles_name";
        $result = $conn->query($listing_sql);
        if ($result->num_rows > 0) {
            while($rows = $result->fetch_assoc()) {
                //Properties
