...
$sql  = "SELECT * FROM `intro_slider` order by `is_sort` ASC";
				...
				if($category_level==1){
                   ...
                   // $results = $conn->query($listing_sql);
            		//if ($results->num_rows > 0) {
							...
							//if($row_one['detailed_mpd']){
							
							$listing_sql = "SELECT a2t.*,ad.articles_url, a.articles_id, a.authors_id, a.articles_date_added, ad.articles_name, ad.articles_head_desc_tag, ad.articles_description, au.authors_name, sn.detailed_mpd,sn.stone_name_id, a2t.topics_id, SUBSTRING( ad.articles_name ,1 ,1) AS start_letter FROM (articles a, articles_description ad, articles_to_topics a2t) LEFT JOIN authors au ON a.authors_id = au.authors_id LEFT JOIN stone_names sn ON a.articles_id = sn.summary_mpd WHERE (a.articles_date_available IS NULL OR to_days(a.articles_date_available) <= to_days(now())) AND a.articles_status = '1' AND a.articles_id = a2t.articles_id AND ad.articles_id = a2t.articles_id AND ad.language_id = '1' AND a2t.topics_id = '3' AND sn.detailed_mpd=".$row_one['detailed_mpd']." GROUP BY a.articles_id ORDER BY ad.articles_name";
        
                    $listing_result = $conn->query($listing_sql);
            		if ($listing_result->num_rows > 0) {
            		    while($rows = $listing_result->fetch_assoc()) {
            		        
            		         if($rows['detailed_mpd']==0){
            		           $mpd_query = "SELECT * FROM `stone_names` WHERE `stone_name` LIKE '" . $rows['articles_name'] . "'";
            		           $resultMpd = $conn->query($mpd_query);
            		           $rowMpd = $resultMpd->fetch_assoc();
            		           if($rowMpd['detailed_mpd']!=0){
            		           $rows['detailed_mpd']=$rowMpd['detailed_mpd'];
            		           $rows['stone_name_id']=$rowMpd['stone_name_id'];
            		           }
            		       }
            		        if($rows['detailed_mpd']){
            		            
            		        $stone_name_query = "select stone_name, stone_name_id from stone_names where `detailed_mpd`= '" . $rows['detailed_mpd'] . "' order by stone_name_id ASC";
...
if($stone_name_ids2){
	...
	foreach($stone_name_ids2 as $stone_name_ids){$pro_final_string='';
		...
		if($prosql_tags->num_rows > 0){
			...
			$chakra = explode(',', $pro_array[2]);
		    $astro = explode(',', $pro_array[5]);
			$pro_final_string.='Primary Chakra - '.$chakra[0].','.$chakra[1].',';
			$pro_final_string.='Astrological Sign - '.$astro[0].','.$astro[1];
...
if($stone_name_ids2){
	...
	foreach($stone_name_ids2 as $stone_name_ids){
		...
		if($prosql_tags->num_rows > 0){
			...
			array_push($proFinalArray,$pro_final_array);
		}
	}
}else{
...
";
							    ...
							    if ($results->num_rows > 0) {
							       ...
							       $linked_product_query = $conn->query($linked_product_query);
							        ...
							        //unit
							        $unittype = '';
				if( $products_options_values['selling_unit_type'] == 'Bag'){
				    $price_tip = str_replace(' ea', '/bag', $price_tip);
					$unittype = 'bag';

				}elseif( $products_options_values['selling_unit_type'] == 'Case'){
				    $price_tip = '/case';
				    $unittype = 'case';
				}elseif( $products_options_values['selling_unit_type'] == 'Unit'){
				    $price_tip = '/unit';
					$unittype = 'unit';
				}elseif( $products_options_values['selling_unit_type'] == 'Bottle'){
				    $price_tip = '/bottle';
					$unittype = 'bottle';
				}elseif( $products_options_values['selling_unit_type'] == 'Set'){
				    $price_tip = '/set';
					$unittype = 'set';
				}elseif( $products_options_values['selling_unit_type'] == 'Box'){
				    $price_tip = '/box';
					$unittype = 'box';
				}elseif( $products_options_values['selling_unit_type'] == 'EBook'){
				    $price_tip = ' Per EBook';
					$unittype = 'EBook';
				}elseif( $products_options_values['selling_unit_type'] == 'Session'){
				    $price_tip = ' Per Session';
					$unittype = 'Session';
				}elseif( $products_options_values['selling_unit_type'] == 'EClass'){
				    $price_tip = ' Per EClass';
					$unittype = 'EClass';
				}else{
					$unittype = 'pcs';
					$price_tip = ' ea';
							        ...
							        //unit End
							          if($products_options_values['products_attributes_units']>0){
							            ...
							            $flag=1;
							             ...
							             $special_saving = number_format(((($products_options_values['product_attribute_price_1'])/$products_options_values['options_values_price'])*100),0);
							            $special_saving =100-$special_saving;
							            if($special_saving!=100 && $special_saving!=0){
							               $special_saving ="(Save ".$special_saving."%) ";
							            }
							               $attribute_1    =$special_saving.'1'. ($products_options_values['product_attribute_qty_1'] == '' ? ' or more -' :' - '. $products_options_values['product_attribute_qty_1'].' '. $unittype.' - ').$strick.' $'.number_format($products_options_values['product_attribute_spe_price_1']>0?$products_options_values['product_attribute_spe_price_1']:$products_options_values['product_attribute_price_1'], 2, '.', ''). $price_tip;
							            }
							            
							            if($products_options_values['product_attribute_price_2']>0){
							                $flag=1;
							                 //percentage
							            $special_saving = number_format(((($products_options_values['product_attribute_price_2'])/$products_options_values['options_values_price'])*100),0);
							            $special_saving =100-$special_saving;
							            if($special_saving!=100 && $special_saving!=0){
							               $special_saving ="(Save ".$special_saving."%) ";
							            }
							               if($products_options_values['product_attribute_qty_1']+1<$products_options_values['products_attributes_units']){ 
							            $attribute_2    =$special_saving. ($products_options_values['product_attribute_qty_1']+1).($products_options_values['product_attribute_qty_2'] == '' ? ' or more -' : ' - '.$products_options_values['product_attribute_qty_2'].' '.$unittype.' - ').$strick.' $'.number_format($products_options_values['product_attribute_spe_price_2']>0?$products_options_values['product_attribute_spe_price_2']:$products_options_values['product_attribute_price_2'], 2, '.', ''). $price_tip;
							               }
							           }
							            
							            if($products_options_values['product_attribute_price_3']>0){
							                $flag=1;
							                 $special_saving = number_format(((($products_options_values['product_attribute_price_3'])/$products_options_values['options_values_price'])*100),0);
							                 $special_saving =100-$special_saving;
							            if($special_saving!=100 && $special_saving!=0){
							               $special_saving ="(Save ".$special_saving."%) ";
							            }
							             if($products_options_values['product_attribute_qty_2']+1<$products_options_values['products_attributes_units']){
							              $attribute_3    =$special_saving.($products_options_values['product_attribute_qty_2']+1).($products_options_values['product_attribute_qty_3'] == '' ? ' or more -' :' - '. $products_options_values['product_attribute_qty_3'].' '.$unittype.' - ').$strick.' $'.number_format($products_options_values['product_attribute_spe_price_3']>0?$products_options_values['product_attribute_spe_price_3']:$products_options_values['product_attribute_price_3'], 2, '.', ''). $price_tip;
							            ...
							            if($products_options_values['product_attribute_price_4']>0){
							                $flag=1;
							                 $special_saving = number_format(((($products_options_values['product_attribute_price_4'])/$products_options_values['options_values_price'])*100),0);
							                 $special_saving =100-$special_saving;
							            if($special_saving!=100 && $special_saving!=0){
							               $special_saving ="(Save ".$special_saving."%) ";
							            }
							            if($products_options_values['product_attribute_qty_3']+1<$products_options_values['products_attributes_units']){
							              $attribute_4    = $special_saving. ($products_options_values['product_attribute_qty_3']+1).($products_options_values['product_attribute_qty_4'] == '' ? ' or more -' :' - '. $products_options_values['product_attribute_qty_4'].' '.$unittype.' - '). $strick.' $'.number_format($products_options_values['product_attribute_spe_price_4']>0?$products_options_values['product_attribute_spe_price_4']:$products_options_values['product_attribute_price_4'], 2, '.', ''). $price_tip;
							            ...
							            if($products_options_values['product_attribute_price_5']>0){
							                $flag=1;
							                $special_saving = number_format(((($products_options_values['product_attribute_price_5'])/$products_options_values['options_values_price'])*100),0);
							                 $special_saving =100-$special_saving;
							            if($special_saving!=100 && $special_saving!=0){
							               $special_saving ="(Save ".$special_saving."%) ";
							            }
							            if($products_options_values['product_attribute_qty_4']+1<$products_options_values['products_attributes_units']){
							               $attribute_5    =  $special_saving. ($products_options_values['product_attribute_qty_4']+1).($products_options_values['product_attribute_qty_5'] == '' ? ' or more -' :' - '. $products_options_values['product_attribute_qty_5'].$unittype.' - '). $strick.' $'.number_format($products_options_values['product_attribute_spe_price_5']>0?$products_options_values['product_attribute_spe_price_5']:$products_options_values['product_attribute_price_5'], 2, '.', ''). $price_tip;
							             }
							                
							            }
							             if($products_options_values['products_attributes_special_price']>0 && $products_options_values['product_attribute_spe_price_1']==0.000  && $products_options_values['product_attribute_spe_price_2']==0.000  && $products_options_values['product_attribute_spe_price_3']==0.000  && $products_options_values['product_attribute_spe_price_4']==0.000  && $products_options_values['product_attribute_spe_price_5']==0.000 ){
							                 $flag=1;
							                 $special_saving = number_format(((($products_options_values['products_attributes_special_price'])/$products_options_values['options_values_price'])*100),0);
							                 $special_saving =100-abs($special_saving);
							            if($special_saving!=100 && $special_saving!=0){
							               $special_saving ="(Save ".$special_saving."%) ";
							            }
							              $attribute_ext    =  $special_saving. ' 1 '.$unittype.' or more '. $strick.' $'.number_format($products_options_values['products_attributes_special_price'], 2, '.', ''). $price_tip;
							             
							                 
							             }
							             if($flag==0){
							                 $attribute_6= '1 '.$unittype.' or more - '.' $'.number_format($products_options_values['options_values_price'], 2, '.', ''). $price_tip;
							             }
							            ...
							            $attribute_string='';
							            if($attribute_1){
							                $attribute_string.=$attribute_1.'<br>';
							            }if($attribute_2){
							                $attribute_string.=$attribute_2.'<br>';
							            }if($attribute_3){
							                $attribute_string.=$attribute_3.'<br>';
							            }if($attribute_4){
							                $attribute_string.=$attribute_4.'<br>';
							            }if($attribute_5){
							                $attribute_string.=$attribute_5.'<br>';
							            }if($attribute_ext){
							                $attribute_string.=$attribute_ext.'<br>';
							            }if($attribute_6){
							                $attribute_string.=$attribute_6.'<br>';
							            }
							            $priceArr   = array('heading'=>$products_attributes_name,
							            'save'=>$attribute_string.($products_options_values['products_attributes_units'] < 15 ? '*Limited Quantity - '.$products_options_values['products_attributes_units'].' units left in Stock' : ''),
							...
							$string = preg_replace('/[\x00-\x1F\x7F-\xFF]/', '', $row['products_description']);*/
							//product url
							$url_str = str_replace(" ","_",$row['products_name']);
							$rewrite_url = preg_replace('/[^A-Za-z0-9_-]+/', '_', $url_str);
							$product_url = base_url.$rewrite_url.'.html';
							
							   
							$txt_format=preg_replace('/[\x00-\x1F\x7F-\xFF]/', '', $row['products_description']);
							   $string = preg_replace('#<a.*?>(.*?)</a>#i', '\1', $txt_format);
								$cat1=array('category_id'=>$row['products_id'],
								'slider'=>1,
								'category_name'=>$row['products_name'],
						...
						'products_image_alt_12' => base_url.'images/'.$row['products_image_alt_12'],
								'category_price'=>'0.00','category_properties'=>$proFinalArray,
								'component'=>'product',
				...
				if($category_level==1){
				  ...
				  asort($stonesArray);
					 ...
					 $conn->close();
						...
						if ($results->num_rows > 0) {
							...
							while($rows = $results->fetch_assoc()) {
								...
								$cat1=array('category_id'=>$rows['products_id'],'slider'=>1,'category_level'=>6,'category_name'=>htmlentities($rows['products_name']),'category_image'=>base_url.'images/'.$rows['products_image'],'component'=>'product');
								array_push($categoryArr['data'],$cat1);
							    }
					 ...
					 $conn->close(); 
					    ...
					    $cat1=array('category_id'=>'A','slider'=>2,'category_level'=>9,'category_name'=>'Crystal & MineralAssortments','component'=>'products');
					    $cat2=array('category_id'=>'C','slider'=>2,'category_level'=>9,'category_name'=>'Cut & PolishedCrystals','component'=>'products');
...
//Properties
			        ...
			        //Stone name
						...
						if ($result_stone->num_rows > 0) {
			        		...
			        		//$stone_name_query = "select stone_name_id from products_to_stones where products_id = '" . $category_id . "' order by stone_name_id ASC";
			        		$stone_name_query = "select stone_name, stone_name_id from stone_names where `detailed_mpd`= '" . $category_id . "' order by stone_name_id ASC";
$no_of_stones = $conn->query($stone_name_query);
	$stone_name_ids2 = array();
		while($stone_name_ids_new = $no_of_stones->fetch_assoc()){
		    $stone_name = $stone_name_ids_new['stone_name'];		    
			$stone_name_ids2[] = $stone_name_ids_new['stone_name_id'];
		}
        ...
        $listing_sql_one = "select a.articles_id, a.articles_date_added, a.articles_date_available, a.authors_id, ad.articles_name, ad.articles_description, ad.articles_url, au.authors_name, a.articles_video from (articles a, articles_description ad) left join authors au on a.authors_id = au.authors_id where a.articles_id = ".$category_id." and ad.articles_id = a.articles_id and ad.language_id = '1'";
            		...
            		if ($results->num_rows > 0) {
            		    while($rows = $results->fetch_assoc()) {
            		       ...
            		       $articles1 = str_replace('Click here to see <a href="https://www.healingcrystals.com/advanced_search_result.php','<a style="display:none" href="https://www.healingcrystals.com/advanced_search_result.php ',$articles1);
            		       $articles1 = str_replace('Click here to see <a href="http://www.healingcrystals.com/advanced_search_result.php','<a style="display:none" href="https://www.healingcrystals.com/advanced_search_result.php ',$articles1);
            		       ...
            		       $articles2 = str_replace('<a href="https://www.healingcrystals.com/Metaphysical','<a style="display:none" href="http://www.healingcrystals.com/Metaphysical',$articles1);
            		       $articles2 = str_replace('<a href="/Metaphysical','<a style="display:none" href="http://www.healingcrystals.com/Metaphysical',$articles1);
			       ...
			       $sql="select pts.stone_name_id, stone_name,detailed_mpd, products_id from products_to_stones pts, stone_names s, stone_properties sp where sp.stone_name_id = pts.stone_name_id and s.stone_name_id = pts.stone_name_id and sp.property_id = '8' and products_id in (select p.products_id from products_specific_property psp, products p where psp.products_id = p.products_id and p.products_status = 1 and (p.products_model like '%' or p.products_model like 'D%') and lower(property_name) = 'color' and property_value = '".$category_id."') group by stone_name_id order by stone_name  limit $startindex,$perpage";
            		...
            		if ($result->num_rows > 0) {
            		    $stoneNameArr=array();
    							       ...
    							       // echo $row_two['products_id'].',';
    							         if(tep_get_product_quantity($row_two['products_id']) != false){
    							             if(!in_array($row_fetch['detailed_mpd'], $stoneNameArr)){
    							                 if(!empty($row_fetch['detailed_mpd'])){
							...
							$listing_sql = "SELECT a2t.*,ad.articles_url, a.articles_id, a.authors_id, a.articles_date_added, ad.articles_name, ad.articles_head_desc_tag, ad.articles_description, au.authors_name, sn.detailed_mpd,sn.stone_name_id, a2t.topics_id, SUBSTRING( ad.articles_name ,1 ,1) AS start_letter FROM (articles a, articles_description ad, articles_to_topics a2t) LEFT JOIN authors au ON a.authors_id = au.authors_id LEFT JOIN stone_names sn ON a.articles_id = sn.summary_mpd WHERE (a.articles_date_available IS NULL OR to_days(a.articles_date_available) <= to_days(now())) AND a.articles_status = '1' AND a.articles_id = a2t.articles_id AND ad.articles_id = a2t.articles_id AND ad.language_id = '1' AND a2t.topics_id = '3' AND sn.detailed_mpd=".$val." ORDER BY ad.articles_name limit 1";
     ...
     if (isset($postdata)) {
		...
		if ($result->num_rows > 0) {
			...
			array_push($categoryArr['data'],$cat);
							...
							$listing_sql = "SELECT a2t.*,ad.articles_url, a.articles_id, a.authors_id, a.articles_date_added, ad.articles_name, ad.articles_head_desc_tag, ad.articles_description, au.authors_name, sn.detailed_mpd,sn.stone_name_id, a2t.topics_id, SUBSTRING( ad.articles_name ,1 ,1) AS start_letter FROM (articles a, articles_description ad, articles_to_topics a2t) LEFT JOIN authors au ON a.authors_id = au.authors_id LEFT JOIN stone_names sn ON a.articles_id = sn.summary_mpd WHERE (a.articles_date_available IS NULL OR to_days(a.articles_date_available) <= to_days(now())) AND a.articles_status = '1' AND a.articles_id = a2t.articles_id AND ad.articles_id = a2t.articles_id AND ad.language_id = '1' AND a2t.topics_id = '3' AND sn.detailed_mpd=".$product_id." GROUP BY a.articles_id ORDER BY ad.articles_name";
...
function metaphysical_buy(){
    ...
    $user_app_id='';
		$action='Accessed buy button for product ID'.$category_id;
		$event='Buy button';
   ...
   $servername = "localhost";
		    ...
		    while($row = $result->fetch_assoc()) {
            	...
            	//	if ($results->num_rows > 0) {
			        ...
			        $userArr                = array('user_id'=>$row['customers_id'],
			                                        'user_name'=>$row['customers_firstname'].' '.$row['customers_lastname'],
			                                        'token'=>$app_key);
					array_push($categoryArr['data'],$userArr);
					$action='Login Success for user'.$row['customers_firstname'].' '.$row['customers_lastname'].' User ID '.$row['customers_id'];
					traceability_api_trigger($user_app_id,$action,$event,$location);
    ...
    function edit_account(){
		...
		if ($auth_result->num_rows > 0) {
         ...
         /* $email_address   = "SELECT * FROM `customers` WHERE `customers_email_address` LIKE '".$email_address."'";
    		$email_result    = $conn->query($email_address);
		if ($email_result->num_rows > 0) {
		     $categoryArr['status']='Error';
		    $categoryArr['error']="Email Already Exists";
...
function add_to_cart(){
    ...
    if (isset($postdata)) {
        ...
        $action='Added product with ID-'.$product_id.', QTY-'.$quantity.', Price-'.$product_price;
		traceability_api_trigger($user_app_id,$action,$event,$location);
   ...
   $servername = "localhost";
    ...
    if (isset($postdata)) {
		...
		$app_key                = customer_pass_encryption($app_id);
	 	...
	 	$sql                    = "SELECT * FROM `customer_mobile_app` WHERE `cma_customers_app_id` LIKE '".$app_id."' limit 1";
		$result                 = $conn->query($sql);
		if ($result->num_rows > 0) {
		    $sql2 = "update customer_mobile_app set cma_push_token='".$push_key."'  where cma_customers_app_id LIKE '".$app_id."'";
             $conn->query($sql2);
            $categoryArr['status']  = 'Success';
		    	$userArr                = array('token'=>$app_key,'push_key'=>$push_key);
		    	array_push($categoryArr['data'],$userArr);
		}else{
			...
			$categoryArr['status']  = 'Success';
			$userArr                = array('user_id'=>$row['customers_id'],
						            'token'=>$app_key,'push_key'=>$push_key);
			array_push($categoryArr['data'],$userArr);
...
function notification_count(){
   ...
   $servername = "localhost";
	...
	//	if ($auth_result->num_rows > 0) {
    	...
    	$sql="select count(cn_id) as notification_count from customer_mobile_notification where cn_user_id = ".$user_id." and cn_status=0";
        $result = $conn->query($sql);
    	if ($result->num_rows > 0) {
    	    while($row = $result->fetch_assoc()) {
    	    $categoryArr['status']  = 'Success';
			$notfyArr                = array('notification_count'=>$row['notification_count']);
			array_push($categoryArr['data'],$notfyArr);
    	    }
    	 }else{
    		    $categoryArr['status']	= "error";
    		    $categoryArr['error']	        = "Incorrect User";
...
function notification(){
    ...
    $event='Notification menu';
        $action='User with ID-'.$user_id.' accessed notification menu';
		traceability_api_trigger($user_app_id,$action,$event,$location);
  ...
  $sql = "SELECT * FROM `customer_mobile_app`";
			...
			if($results->num_rows > 0){
				...
				while($customerArr = $results->fetch_assoc()){
				    echo '<tr>';
				    echo '<td>'.$i.'</td>';
				    echo '<td><a href="'.base_url.api_path.'check2?user='.$customerArr['cma_customers_app_id'].'">'.$customerArr['cma_customers_app_id'].'</a></td>';
...
";
				...
				if( $products_options_values['selling_unit_type'] == 'Bag'){
					...
					$price_tip = ' ea';
							    ...
							    //-----------------------------------------------------------------------------------------
							   echo  'Amethyst - Amethyst Druze Clusters (Brazil)'.'<br>';
							   $url_str = str_replace(" ","_",'Tourmaline - Black Tourmaline Chips/Chunks "Extra" (Brazil)');
							   $rewrite_url = preg_replace('/[^A-Za-z0-9_-]+/', '_', $url_str);
							   echo $product_url = base_url.$rewrite_url.'.html';die();
							  /*// $txt_format=preg_replace('/[\x00-\x1F\x7F-\xFF]/', '', $row['products_description']);
							   //$string = preg_replace('#<a.*?>(.*?)</a>#i', '\1', $txt_format);*/
							   die();
							    $dd=strip_tags($row['products_description']);
